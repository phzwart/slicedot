#!/usr/bin/env python3
"""Download peptide-inhibitor references (CCD / PubChem) into per-ligand subdirectories.
...
def _parse_sdf(path: Path) -> dict:
    ...
    # Prefer element+serial naming that matches common ligand atom ids when possible.
...
def topology_from_parsed(parsed: dict, *, label: str, slug: str) -> dict:
    ...
    # No steroid A-ring planar heuristic for peptidomimetics.
...
def main():
    ...
    print(f"building {len(entries)} peptide-inhibitor ligand refs …", flush=True)
    for e in entries:
        write_ligand_dir(e)
