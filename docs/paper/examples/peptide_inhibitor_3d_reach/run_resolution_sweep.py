#!/usr/bin/env python3
"""Run free-OT → Namer → ADMM cleanup for each peptide inhibitor at several resolutions.
...
def main():
    ...
    ap.add_argument(
        ...
        help="Comma-separated Å values (default: 3.0).",
    )
    ap.add_argument("--n-seeds", type=int, default=1)
    ap.add_argument("--seed0", type=int, default=0)
    ap.add_argument("--atom-factor", type=float, default=2.0,
                    help="Overcomplete free-OT atom factor (default 2.0).")
    ap.add_argument(
    ...
    print(
        f"peptide-inhibitor sweep: {len(entries)} ligands × {len(resolutions)} resolutions "
